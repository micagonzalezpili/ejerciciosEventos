...
boton.addEventListener("click", function (e) {
  e.preventDefault();
  let pesoUsuario = peso.value;
  let alturaUsuario = estatura.value;
  let imc = pesoUsuario / ((alturaUsuario / 100) ** 2);
  return resultado.innerText = `Tu IMC es ${imc.toFixed(2)}`;
})

// EJERCICIO 2 //
let pesoArInput = document.getElementById("pesoAr")
let dolarUsInput = document.getElementById("dolarUs")
let resultadoDos = document.getElementById("resultadoDos")
let formulario = document.getElementById("form")

formulario.addEventListener("submit", (e) => {
  e.preventDefault();
  let pesos = parseFloat(pesoArInput.value);
  let dolares = parseFloat(dolarUsInput.value);
  if (pesos) {
    pesoArInput.value = ""
    let mensaje = `El valor es de ${cambioPesos(pesos).toFixed(2)}`
    resultadoDos.textContent = mensaje
  }
  if (dolares) {
    dolarUsInput.value = ""
    let mensaje = `El valor es de ${cambioDolares(dolares).toFixed(2)}`
    resultadoDos.textContent = mensaje
  }
})
...
function cambioDolares(monedaDos) {
  const usd = 140;
  return monedaDos * usd;
} */
